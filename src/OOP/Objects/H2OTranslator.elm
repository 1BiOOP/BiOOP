...
import OOP.LangUtils exposing (mergeEnv, mergeState)
...
htmlMatch html hp oenv dict userDefTemps state =
    case (html, hp) of
        ...
        (VString _ _, TBPrim _ Cat _ _) ->
            Result.Ok (oenv, state)

        (VString _ _, TString _ _ _) ->
            if equalVStrAndTStr html hp then
                Result.Ok (oenv, state)
            else
        ...
        VCons v1 v2 ->
            case v1 of
                VCons v11 (VCons v12 VNil) ->
                    if v11 == VString (VChar 'i') (VString (VChar 'd') VEmpStr) 
                    then
...
equalVStrAndTStr vstr tstr =
    case (vstr, tstr) of
        (VString (VChar v1) vs2, TString _ (TChar _ t1) ts2) ->
            if v1 == t1 then
                equalVStrAndTStr vs2 ts2
            else 
                False
