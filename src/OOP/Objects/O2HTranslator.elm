...
import OOP.Parser.Term exposing (stringToTerm)
import OOP.Parser.Value exposing (stringToValue)
import OOP.Parser.Value exposing (valueListToVCons)
import OOP.Syntax exposing (Value(..), Env, Term(..), Bop(..), State)
import OOP.LangUtils exposing (appendTermString, strToTString, appendValueString)
        ...
        TList ws1 (TList ws2 t1 (TList ws3 t2 (TEmpList ws4))) (TEmpList ws5) ->
            ...
            in
                TList ws1 (TList ws2 t1 (TList ws3 t2_ (TEmpList ws4))) 
                    (TList [] 
                        (TList [] (stringToTerm [] <| toList "class")
                                (TList [] (stringToTerm [] <| toList "Add")
                                    (TEmpList [])))
                    (TEmpList ws5))
    ...
    case attr of
            VCons id (VCons (VCons class (VCons val VNil)) VNil) ->
                VCons id (VCons (VCons class (VCons
                    (appendValueString val (stringToValue <| toList " Add"))
                VNil)) VNil)
...
addObjectID id object =
    VHtml "div" VNil
        (valueListToVCons
        <| [ (valueListToVCons [stringToValue  <| toList "id",
                                stringToValue <| toList id])
            ,(valueListToVCons [stringToValue <| toList "class",
                                stringToValue <| toList "Object"])])
        (VCons object VNil)