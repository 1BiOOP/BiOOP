...
import OOP.LangUtils exposing (processAfterParse)
...
import OOP.Syntax exposing (OOP(..), Value(..), ClassTable, State)
        ...
        case (parseProgramRes, outputRes) of
            (Result.Ok (WithCT classtb term), Result.Ok (output, newState)) ->
                let
                    ...
                    (ctx_, term_) =
                        uneval {env = [], state = [], classtb = withGui} pTerm 
            ...
            (Result.Ok (NoCT term), Result.Ok (output, newState)) ->
                let
                    ...
                    (_, term_) =
                        uneval {env = [], state = [], classtb = parsedGui} pTerm 
                        <| {value = output, state = newState}
