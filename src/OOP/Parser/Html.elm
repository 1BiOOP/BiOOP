...
import OOP.LangUtils exposing (strToVString)
import String exposing (toList, trim, split)
            ...
            let
                ...
                proItem =
                    case nameAndValue of
                        n :: val :: [] ->
                            VCons (n |> trim |> toList |> strToVString)
                                    (VCons (trim val |> toList |> strToVString) VNil)
...
parseOtherPro al =
    case al of
        ...
        (name, value) :: al_ ->
            let
                proItem =
                    VCons (name |> trim |> toList |> strToVString)
                    (VCons (value |> trim |> toList |> strToVString)
                        VNil)
            in
